...
/**
 * A server that speaks just enough SMTP to have a conversation with, and remembers everything said.
 *
 * `ehlo` is given as the lines the server sends, and the *last* one has to be the final one — every
 * earlier line ends with `-` and not a space, or a client parsing correctly will treat the first line
 * as the whole reply. The first version of this helper got that wrong, and six tests failed in ways
 * that looked like bugs in the client.
 */
async function fakeServer(t, { greeting = '220 fake ESMTP', ehlo = ['250 fake'], authReply = '235 ok', recipientReply = '250 ok', afterStartTls = 'close' } = {}) {
  const seen = { conversation: [], message: null, auth: null, credentials: null };
  ...
  const server = createServer((socket) => {
    ...
    const handle = (line) => {
      ...
      if (login) {
        ...
        login -= 1;
        socket.write(login === 0 ? `${authReply}\r\n` : '334 continue\r\n');
        return;
      }
      ...
      if (upper === 'STARTTLS') {
        ...
        if (afterStartTls === 'close') socket.end();
        else socket.write('this is not a TLS handshake\r\n');
        return;
      }
